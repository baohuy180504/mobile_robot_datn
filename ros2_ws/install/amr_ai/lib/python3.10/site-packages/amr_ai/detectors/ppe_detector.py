...
class PPEDetector:
    ...
    def _class_to_type(self, raw_name):
        ...
        # ── Helmet ────────────────────────────────────────────────────────────
        has_helmet_kw = "helmet" in name or "hardhat" in name or "hard-hat" in name
        if has_helmet_kw:
            helmet_pos = min(
            ...
            no_pos = name.find("no")
            # "no" xuất hiện TRƯỚC "helmet" → là class "no_helmet"
            if no_pos != -1 and no_pos < helmet_pos:
                return "no_helmet"
            return "helmet"

        # ── Vest ──────────────────────────────────────────────────────────────
        if "vest" in name:
            vest_pos = name.find("vest")
            no_pos = name.find("no")
            # "no" xuất hiện TRƯỚC "vest" → là class "no_vest"
            # Bắt được: "no-vest", "novest", "no-safety-vest", "no-vis-vest", v.v.
            if no_pos != -1 and no_pos < vest_pos:
                return "no_vest"
            return "vest"
