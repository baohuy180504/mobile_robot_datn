...
<style>
  ...
  *{box-sizing:border-box;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none;}
  ...
  .pad{display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-rows:72px 72px 72px;
    gap:9px;margin:4px 0;}
  .pad .blank{visibility:hidden;}
  .pad button{font-size:28px;border-radius:16px;border:1px solid var(--border);
    background:#0b1220;color:var(--text);cursor:pointer;touch-action:none;
    display:flex;align-items:center;justify-content:center;transition:color .15s,border-color .15s,box-shadow .15s;}
  .pad button:disabled{opacity:.3;cursor:not-allowed;}
  .pad button:active:not(:disabled){background:#1e293b;transform:scale(.96);}
  .pad .stop-symbol{color:var(--red);font-size:22px;}
  .pad .arrow-btn:not(:disabled){color:var(--cyan);border-color:var(--cyan);
...
<div class="page">
  ...
  <div class="pad" id="padArea" oncontextmenu="return false;">
    <div class="blank"></div>
    <button id="btnUp" class="arrow-btn" disabled title="Tien"
      onpointerdown="beginMove('forward')" onpointerup="endMove()" onpointerleave="endMove()" onpointercancel="endMove()">&#9650;</button>
    <div class="blank"></div>

    <button id="btnLeft" class="arrow-btn" disabled title="Quay trai"
    ...
    <button id="btnRight" class="arrow-btn" disabled title="Quay phai"
      onpointerdown="beginMove('right')" onpointerup="endMove()" onpointerleave="endMove()" onpointercancel="endMove()">&#9654;</button>

    <div class="blank"></div>
    <button id="btnDown" class="arrow-btn" disabled title="Lui"
...
// Chi chan touchmove (chong keo trang) BEN TRONG vung pad dieu khien -
// truoc day chan toan bo document nen khi noi dung cao hon man hinh,
// nguoi dung khong the cuon xuong de thay thanh truot con lai.
document.getElementById("padArea").addEventListener(
  "touchmove",
...
async function startControl(){
  ...
  setStatus("Dang doi Arduino san sang ...", "busy");
  try{
...
def api_stop():
    ...
    serial_link.write_twist(0.0, 0.0)
    return JSONResponse({"ok": True, "message": "Dang dung dieu khien"})
