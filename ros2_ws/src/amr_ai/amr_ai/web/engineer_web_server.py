...
function removeWaypointRow(id){
  ...
  if(activeWaypointId===id){
    activeWaypointId=waypointRows.length ? waypointRows[waypointRows.length-1].id : null;
  }
  renderWaypointList();
  drawAll();
...
function renderWaypointList(){
  ...
  if(waypointRows.length===0){
    list.innerHTML='<div class="hint">Chưa có waypoint. Nhấn ADD hoặc chọn WAYPOINT rồi click/kéo trên map.</div>';
    return;
  }
  ...
  waypointRows.forEach(row=>{
    ...
    input.onfocus=()=>{ activeWaypointId=row.id; };
...
function getActiveWaypointRow(){
  let row=waypointRows.find(w=>w.id===activeWaypointId);
  if(!row){
    row=waypointRows.length ? waypointRows[waypointRows.length-1] : addWaypointRow();
  }
  return row;
}
...
function setWaypointsOnMap(){
  ...
  const status=document.getElementById("mapToolStatus");
  if(status){
    status.textContent=`WAYPOINT: đã xác nhận ${okCount}/${waypointRows.length} waypoint trên map.`;
  }

  log("Confirmed waypoints: "+okCount+"/"+waypointRows.length);
}
...
function setupCanvasInteraction(){
  canvas.addEventListener("wheel", (e)=>{
    ...
    viewPanX += mx-afterCanvas.x;
    viewPanY += my-afterCanvas.y;
  ...
  window.addEventListener("mousemove", (e)=>{
    if(isPanning && activeMapTool==="PAN"){
      ...
      viewPanX += (e.clientX-lastMouseX)*sx;
      viewPanY += (e.clientY-lastMouseY)*sy;
...
function drawOccupancyLayer(layer, isBase=false){
  ...
  const topLeft=worldToCanvas(origin.x, origin.y+hMeters);
